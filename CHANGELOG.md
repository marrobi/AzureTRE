...
* Seed and finalise closed cost months with the [Cost Management Exports API](https://learn.microsoft.com/azure/cost-management-billing/automate/tutorial-seed-historical-cost-dataset-exports-api) instead of repeated Query API calls. One one-time monthly export delivers a whole month of daily, resource-level costs as a single CSV to a new `cost-exports` container, which the Cost Processor ingests via a new internal API endpoint; the still-settling current month remains on the Query API. This avoids the Query API's one-year-per-request limit and the throttling caused by many historical queries ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))

* Include costs tagged with a deleted resource's id in cost reports. Reports are built from the tags Azure recorded at the time the cost was incurred, not from the resources that still exist, so decommissioned workspaces and shared services no longer silently drop out of historical totals ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
* Stream the Cost Management export CSV instead of loading it into memory, so ingesting a large month's export no longer scales the Cost Processor's memory use with the size of the export ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
...
BUG FIXES:
* Aggregate only this TRE's `tre_id`-tagged resources from a subscription-wide Cost Management export. Previously any `tre_`-prefixed tag was ingested, so in a subscription shared with other TREs their workspace/shared-service costs leaked into this TRE's report (surfaced as `unattributed`); resources not tagged with this `tre_id` are now excluded ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
...
* Only read the export manifest written by the run just executed, and treat a run reporting no data as an empty month without looking for one. Every run of a month reuses the same deterministic export name, so falling back to an earlier run's manifest could re-ingest a previous run's costs as if they were current ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
* Post a large month's exported costs as several contiguous day ranges instead of a single request, so a month exceeding the ingest row cap is collected rather than rejected outright. Requests are always split on a day boundary because ingest replaces a day's stored costs rather than merging into them ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
* Expire delivered Cost Management export CSVs after a configurable retention period (`cost_processor_export_retention_days`, default 7 days). Every export run writes a new folder and the backfill and finalisation timers re-run exports daily, so the delivered CSVs previously accumulated in the `cost-exports` container indefinitely ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
* Wait for each Cosmos container create/update to finish during API startup. The operation was started but never awaited, so the API could report a successful bootstrap and begin serving requests while a container was still being created, and failures of the operation were never surfaced ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
* Store collected cost data as one document per day per scope rather than one per report period, and derive Monthly *and* ungranular reports from those days. A single Daily collection now serves every granularity (previously the default ungranular report never matched a collection key and always re-queried Cost Management), substantially reducing document size, the frequently re-run current-month refresh only rewrites the days that changed, and an arbitrary report range composes from whole days instead of persisting a document of its own ([#2350](https://github.com/microsoft/AzureTRE/issues/2350))
