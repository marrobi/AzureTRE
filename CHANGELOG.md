...
ENHANCEMENTS:
* Grant the workspace owners and researchers groups the `Key Vault Secrets User` role on the workspace Key Vault so researchers can read secrets stored by workspace services (e.g. Gitea/OHDSI database passwords). (`tre-workspace-base` 2.9.0) ([#4992](https://github.com/microsoft/AzureTRE/issues/4992))
