...
ENHANCEMENTS:
* Redesign the airlock storage architecture around two shared storage accounts — external-facing core (`stalairlock{tre_id}`) and internal workspace-global (`stalairlockg{tre_id}`) — using container metadata to track request stages instead of 5–10 per-workspace accounts. A per-workspace `airlock_version` property (1=legacy, 2=consolidated) plus a core-level `enable_legacy_airlock` toggle provide a gradual, backwards-compatible migration (replacing the `USE_METADATA_STAGE_MANAGEMENT` env var), with API guardrails that block creating an `airlock_version=1` workspace while legacy airlock is disabled and block changing `airlock_version` while in-flight requests exist. Cross-workspace isolation on the shared accounts is enforced by per-workspace private endpoints, per-workspace private DNS zones, and ABAC conditions: the global account uses a per-workspace SAS signer app registration (workload identity federation from the core API managed identity) matching `workspace_id`, and the import-review identity's access to core `import-in-progress` data is scoped by a `review_workspace_id` metadata match. Changing `airlock_version` via the API must include `enable_airlock: true` in the same PATCH, and PATCH requests are also blocked from setting `airlock_version=1` while legacy airlock is disabled. The per-workspace signer's workload-identity-federation issuer and token-exchange audience are derived from the target cloud so airlock signing works in sovereign clouds. See the [airlock architecture documentation](./docs/azure-tre-overview/airlock.md) for details. (`tre-workspace-base` 3.10.3, `tre-workspace-airlock-import-review` 1.7.0, `tre-service-guacamole-export-reviewvm` 2.0.1, `api` 0.27.5, `airlock_processor` 0.8.16) ([#4964](https://github.com/microsoft/AzureTRE/pull/4964), [#4853](https://github.com/microsoft/AzureTRE/pull/4853), [#4358](https://github.com/microsoft/AzureTRE/issues/4358))
