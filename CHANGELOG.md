...
ENHANCEMENTS:
* Validate the configured `location` before deployment: reject malformed region names in `config_schema.json` and fail fast with a clear message (listing the valid regions) when the value is not an available Azure region for the target subscription/cloud, instead of failing deep in `terraform apply`. (devops 0.6.5) ([#4996](https://github.com/microsoft/AzureTRE/issues/4996))
