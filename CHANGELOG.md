...
ENHANCEMENTS:
* Add an optional `location_options` allow-list to `config.yaml`. When set, the API restricts the workspace `Azure location (region)` to those regions (rejecting others on create) and the UI renders them as a dropdown; when omitted, any region is allowed and workspaces default to the core TRE region. Also tightened the core `location` schema pattern to reject malformed region names. (core 0.16.17, API 0.26.6) ([#4996](https://github.com/microsoft/AzureTRE/issues/4996))
* Allow deploying a workspace (and its workspace services and user resources) to a different Azure region than the core TRE by selecting an `Azure location (region)` in the UI/API. Defaults to the core TRE region when left blank. (API 0.26.1) ([#33](https://github.com/microsoft/AzureTRE/issues/33))

BUG FIXES:
* Give each workspace its own route table instead of reusing the core-region route table, so a workspace deployed to a different Azure region (or subscription) no longer fails associating its subnets with the core-region route table. Note: upgrading an existing workspace re-associates its subnets from the core route table to a new workspace-local one (routing to the firewall is unchanged). (`tre-workspace-base` 2.11.0) ([#33](https://github.com/microsoft/AzureTRE/issues/33))
