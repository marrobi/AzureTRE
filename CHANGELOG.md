...
* Fix the shared Guacamole service to discover all Guacamole workspace services in the authenticated user's workspace and aggregate their user-resource VMs, instead of querying a single static `SERVICE_ID` (which was incorrectly set to the shared service's own resource ID). (`tre-shared-service-guacamole` 0.1.1)
* Fix the shared Guacamole service to honour each workspace's own Guacamole policy (copy/paste, drive, download/upload and keyboard layout) by reading the settings from each Guacamole workspace service's specification per connection, instead of applying the shared service's single deployment-time defaults to every workspace. (`tre-shared-service-guacamole` 0.2.0) ([#13](https://github.com/microsoft/AzureTRE/issues/13))
* Remove the shared Guacamole service's deployment-time policy parameters and environment-variable fallback; each connection's Guacamole policy is now sourced solely from the owning workspace service's specification (Guacamole's built-in defaults apply when unset). (`tre-shared-service-guacamole` 0.3.0) ([#13](https://github.com/microsoft/AzureTRE/issues/13))
...
* Add shared Guacamole service (`tre-shared-service-guacamole`) that enables a single Apache Guacamole instance to serve multiple workspaces with dynamic authentication
