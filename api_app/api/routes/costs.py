...
async def costs(
    ...
    try:
        return await cost_service.query_tre_costs(
            config.TRE_ID, params.granularity, params.from_date, params.to_date, workspace_repo, shared_services_repo, costs_repo)
...
async def refresh_costs(
    ...
    (TRECostProcessor role); the TRE API is the sole writer of the cost collection.
    """
    validate_report_period(params.from_date, params.to_date)
    try:
        ...
        logger.exception("Failed to refresh Azure TRE costs")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=strings.API_GET_COSTS_INTERNAL_SERVER_ERROR)
