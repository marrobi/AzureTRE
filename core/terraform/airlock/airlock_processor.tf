...
resource "azurerm_linux_function_app" "airlock_function_app" {
  ...
  app_settings = {
    "SB_CONNECTION_STRING"                       = var.servicebus_connection_string
    "BLOB_CREATED_TOPIC_NAME"                    = var.blob_created_topic_name
    "TOPIC_SUBSCRIPTION_NAME"                    = var.topic_subscription_name
    ...
    "AIRLOCK_STATUS_CHANGED_QUEUE_NAME"          = var.status_changed_queue_name
    "AIRLOCK_SCAN_RESULT_QUEUE_NAME"             = var.scan_result_queue_name
    "AIRLOCK_DATA_DELETION_QUEUE_NAME"           = var.data_deletion_queue_name
