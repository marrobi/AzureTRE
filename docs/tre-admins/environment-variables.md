...
| `WORKSPACE_REQUESTS_ENABLED` | If set to `true`, enables the workspace request and approval workflow. TRE Users can formally request workspaces which TRE Admins then review and approve/reject. Default value is `false`. See [Workspace Requests](../azure-tre-overview/workspace-requests.md). |
