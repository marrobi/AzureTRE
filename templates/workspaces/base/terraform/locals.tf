locals {
  ...
  unique_identifier_suffix       = var.unique_identifier_suffix != "" ? var.unique_identifier_suffix : local.short_workspace_id
  workspace_resource_name_suffix = "${var.tre_id}-ws-${local.short_workspace_id}"
  storage_name                   = lower(replace("stgws${local.unique_identifier_suffix}", "-", ""))
