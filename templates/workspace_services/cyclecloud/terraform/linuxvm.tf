...
resource "azurerm_role_definition" "offer_reader" {
  name        = "Offer Reader"
  scope       = data.azurerm_subscription.primary.id
  description = "Ability to read marketpace offers"
...
resource "azurerm_role_assignment" "offer_reader" {
  scope                = data.azurerm_subscription.primary.id
  role_definition_name = "Offer Reader"
  principal_id         = azurerm_virtual_machine.cyclecloud.identity.0.principal_id

  depends_on = [
    azurerm_role_definition.offer_reader
  ]
}
...
resource "azurerm_private_dns_zone" "cyclecloud" {
  name                = "${local.service_resource_name_suffix}.${module.azure_region.location_cli}.cloudapp.azure.com"
  resource_group_name = data.azurerm_resource_group.rg.name
  tags                = local.tre_shared_service_tags
